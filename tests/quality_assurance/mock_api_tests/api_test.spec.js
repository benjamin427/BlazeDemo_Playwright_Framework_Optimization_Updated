...
test('Mock API request to find the name Virgin America, flight number, and time for departure and arrival in the menu', async({page}) => {
    logger.info('tests/quality_assurance/mock_api_tests/api_test.spec.js: Start mocking API requests...')
    ...
    logger.info('tests/quality_assurance/mock_api_tests/api_test.spec.js: Start assertions...')
...
test('Mock API request to find the name United Airlines, flight number, and the time for departurre and arrival in the menu', async ({page}) => {
    logger.info('tests/quality_assurance/mock_api_tests/api_test.spec.js: Start mocking API test for the second test case...')
...
test('Mock API request to find the name Are Lingus, flight number, and the time for departure and arrival in the menu', async({page}) => {
    logger.info('tests/quality_assurance/mock_api_tests/api_test.spec.js: Start mocking API requests for the third test case...')
...
test.afterEach(async({}, testInfo) => {
    if (testInfo.status === 'failed') {
        logger.error(`tests/quality_assurance/mock_api_tests/api_test.spec.js: Test failed at step: "${testInfo.title}"`)
    } else {
        logger.info(`tests/quality_assurance/mock_api_tests/api_test.spec.js: Test complete with status: ${testInfo.status.toUpperCase()} at step: ${testInfo.title}`)
    }
})