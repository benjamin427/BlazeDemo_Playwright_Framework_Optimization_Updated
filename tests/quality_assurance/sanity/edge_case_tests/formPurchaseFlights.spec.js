import { blazedemo_reserve_flights } from '../../../../environments/quality_assurance/sanity/pageObjects/flight_purchase_verification/flight_purchase_verification'
import { blazedemo_purchaseForm } from '../../../../environments/quality_assurance/sanity/pageObjects/purchaseForm/purchaseForm'
import {blazedemo_menuSelectDeparture, blazedemo_menuSelectDestination } from '../../../../environments/quality_assurance/sanity/pageObjects/menu_departure_destination_city/menu_departure_destination_city'
import {test, expect} from '@playwright/test'
import logger from '../../../../utils/logger'

test.beforeEach(async({page}) => {
    const visit = new blazedemo_menuSelectDeparture(page)
    logger.info('tests/quality_assurance/sanity/edge_case_tests/formPurchaseFlights.spec.js: Start test execution initialization...')
...
test.afterEach(async({page}, testInfo) => {
    ...
    if (testInfo.status === 'failed') {
        logger.error(`tests/quality_assurance/sanity/edge_case_tests/formPurchaseFlights.spec.js: Test failed at step: "${testInfo.title}"`)
    } else {
        logger.info(`tests/quality_assurance/sanity/edge_case_tests/formPurchaseFlights.spec.js: Test complete with status: ${testInfo.status.toUpperCase()} at step: ${testInfo.title}`)
    }
