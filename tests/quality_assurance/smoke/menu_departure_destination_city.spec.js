...
test.beforeEach(async({page}) => {
    ...
    logger.info('tests/quality_assurance/smoke/menu_departure_destination_city.js: Starting test execution initialization...')
...
test.afterEach(async({page}, testInfo) => {
    ...
    if (testInfo.status === 'failed') {
        logger.error(`tests/quality_assurance/smoke/menu_departure_destination_city.js: Test failed at step: "${testInfo.title}"`)
    } else {
        logger.info(`tests/quality_assurance/smoke/menu_departure_destination_city.js: Test completed with status: ${testInfo.status.toUpperCase()} at step: ${testInfo.title}`)
    }
})