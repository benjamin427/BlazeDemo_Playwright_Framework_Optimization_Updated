...
test.beforeEach(async({page}) => {
    ...
    logger.info('tests/production/sanity/edge_case_tests/login.spec.js: Start test execution initialization...')
...
test.afterEach(async({page}, testInfo) => {
    ...
    if (testInfo.status === 'failed'){
        logger.error(`tests/production/sanity/edge_case_tests/login.spec.js: Test failed at step: "${testInfo.title}"`)
    } else {
        logger.info(`tests/production/sanity/edge_case_tests/login.spec.js: Test complete with status: ${testInfo.status.toUpperCase()} at step: ${testInfo.title}`)
    }
