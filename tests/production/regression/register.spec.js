import {blazedemo_register} from '../../../environments/production/regression/pageObjects/register/register'
...
test('Enter and submit registration form', async({page}, testInfo) => {
    ...
    logger.info('tests/production/regression/register.spec.js: Start test case initialization...')
    ...
    logger.info('tests/production/regression/register.spec.js: Verifying the url endpoint...')
    await expect(page).toHaveURL(referenceURL)
    logger.info('tests/production/regression/register.spec.js: Entering registration form...')
    ...
    logger.info('tests/production/regression/register.spec.js: Clicking register button...')
    // Click register button
    await register.clickRegisterButton()
    
    // Verifying the status of test results
    if (testInfo.status === 'failed'){
        logger.error(`tests/production/regression/register.spec.js: Test failed at step: "${testInfo.title}"`)
    } else {
        logger.info(`tests/production/regression/register.spec.js: Test complete with status: ${testInfo.status.toUpperCase()} from step: ${testInfo.title}`)
    }
