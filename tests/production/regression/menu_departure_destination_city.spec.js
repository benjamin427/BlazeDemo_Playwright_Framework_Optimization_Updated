...
test.beforeEach(async({page}) => {
    ...
    logger.info('tests/production/regression/menu_departure_destination_city.spec.js: Starting test execution initialization...')
...
test.afterEach(async({page}, testInfo) => {
    ...
    if (testInfo.status === 'failed') {
        logger.error(`tests/production/regression/menu_departure_destination_city.spec.js: Test failed at step: "${testInfo.title}"`)
    } else {
        logger.info(`tests/production/regression/menu_departure_destination_city.spec.js: Test completed with status: ${testInfo.status.toUpperCase()} at step: ${testInfo.title}`)
    }
