import {blazedemo_Login} from '../../../environments/production/smoke/pageObjects/login/login'
...
test('Go to login website and enter login credentials', async({page}, testInfo) => {
    ...
    logger.info('tests/production/smoke/login.spec.js: Starting test execution initialization...')
    ...
    if (testInfo.status === 'failed'){
        logger.error(`tests/production/smoke/login.spec.js: Test has failed at step: "${testInfo.title}"`)
    } else {
        logger.info(`tests/production/smoke/login.spec.js: Test is complete with status: ${testInfo.status.toUpperCase()} at step: ${testInfo.title}`)
    }
