import {blazedemo_menuSelectDeparture} from '../../../environments/production/smoke/pageObjects/menu_departure_destination_city/menu_departure_destination_city'
import {blazedemo_menuSelectDestination} from '../../../environments/production/smoke/pageObjects/menu_departure_destination_city/menu_departure_destination_city'
import {blazedemo_reserve_flights} from '../../../environments/production/smoke/pageObjects/flight_purchase_verification/flight_purchase_verification'
import {test, expect} from '@playwright/test'
import logger from '../../../utils/logger'

test.beforeEach(async({page}) => {
    const departure = new blazedemo_menuSelectDeparture(page)
    logger.info('tests/production/smoke/flight_purchase_verification.spec.js: Start test execution initialization...')
...
test.afterEach(async({page}, testInfo) => {
    ...
    if (testInfo.status === 'failed') {
        logger.error(`tests/production/smoke/flight_purchase_verification.spec.js: Test failed at step: "${testInfo.title}"`)
    } else {
        logger.info(`tests/production/smoke/flight_purchase_verification.spec.js: Test complete with status: ${testInfo.status.toUpperCase()} at step: ${testInfo.title}`)
    }
