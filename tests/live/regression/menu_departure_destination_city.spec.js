import {blazedemo_menuSelectDeparture} from '../../../environments/live/regression/pageObjects/menu_departure_destination_city/menu_departure_destination_city'
import {blazedemo_menuSelectDestination} from '../../../environments/live/regression/pageObjects/menu_departure_destination_city/menu_departure_destination_city'
import {test, expect} from '@playwright/test'
import logger from '../../../utils/logger'

test.beforeEach(async({page}) => {
    const departures = new blazedemo_menuSelectDeparture(page)
    logger.info('tests/live/regression/menu_departure_destination_city.spec.js: Starting test execution initialization...')
...
test.afterEach(async({page}, testInfo) => {
    ...
    if (testInfo.status === 'failed') {
        logger.error(`tests/live/regression/menu_departure_destination_city.spec.js: Test failed at step: "${testInfo.title}"`)
    } else {
        logger.info(`tests/live/regression/menu_departure_destination_city.spec.js: Test completed with status: ${testInfo.status.toUpperCase()} at step: ${testInfo.title}`)
    }
