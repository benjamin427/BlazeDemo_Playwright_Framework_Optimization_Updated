...
test('Enter and submit registration form', async({page}, testInfo) => {
    ...
    logger.info('tests/live/sanity/register.spec.js: Start test case initialization...')
    logger.info('tests/live/sanity/register.spec.js: Go to BlazeDemo.com/register website')
    // Navigate to the website and verify the url endpoint
    await register.visitWebsite()
    logger.info('tests/live/sanity/register.spec.js: Verifying the url endpoint...')
    await expect(page).toHaveURL(referenceURL)
    logger.info('tests/live/sanity/register.spec.js: Entering registration form...')
    ...
    if (testInfo.status === 'failed'){
        logger.error(`tests/live/sanity/register.spec.js: Test failed at step: "${testInfo.title}"`)
    } else {
        logger.info(`tests/live/sanity/register.spec.js: Test complete with status: ${testInfo.status.toUpperCase()} from step: ${testInfo.title}`)
    }
