...
test('Go to login website and enter login credentials', async({page}, testInfo) => {
    ...
    logger.info('tests/live/sanity/login.spec.js: Starting test execution initialization...')
    ...
    if (testInfo.status === 'failed'){
        logger.error(`tests/live/sanity/login.spec.js: Test has failed at step: "${testInfo.title}"`)
    } else {
        logger.info(`tests/live/sanity/login.spec.js: Test is complete with status: ${testInfo.status.toUpperCase()} at step: ${testInfo.title}`)
    }
